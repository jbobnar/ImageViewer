 ...
 *
 * $Date: 2014-06-06 20:04:49 +0200 (Pet, 06 jun. 2014) $
 *
...
public class BasicImageContext extends ImageContext {
	...
	static class VariableWidthFunction extends HorizontalStripFunction {
		...
		VariableWidthFunction(int minX,int maxX,Point2D topLeft,Point2D topRight,Point2D bottomRight,Point2D bottomLeft) throws LineSegmentIntersectionException {
			...
			SortedSet<Point2D> horizontalList = new TreeSet<>( xComparator );
		...
		private static SortedSet<Point> removeRedundantYs(Set<Point> points,boolean useGreaterValue) {
		...
		private static void store(Set<Point> points,int[] xs,int[] ys) {
		...
		private static void addPoint2D(Set<Point> dest,Point2D p,boolean roundUp) {
			int y = (int)p.getY();
	...
	List<TileInstructions> pendingTileInstructions = new LinkedList<>();


	Set<Integer> reservedTiles = new HashSet<>();
